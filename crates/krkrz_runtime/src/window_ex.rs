...
fn interface() -> Result<Interface> {
    Ok(serde_json::from_str(include_str!("../data/window_ex.json"))?)
}
...
impl State {
    pub(crate) fn link(&mut self, vm: &mut Vm, spelling: &str) -> Result<()> {
        ...
        for namespace in api.exports.keys() {
            ...
            let mut object = vm.globals.get(parts.next().unwrap()).cloned()
                .context("Cannot convert the variable type ((void) to Object)")?;
            for part in parts {
                object = vm.get_member(&object, &Value::string(part), false)?;
            }
            ensure!(matches!(object, Value::Object(r) if r.object.is_some()),
        ...
        for (namespace, exports) in api.exports {
            ...
            for name in exports.methods {
                ...
                vm.register_native_method(receiver, &name, &format!("WindowEx.{namespace}.{name}"))?;
            }
            for name in exports.properties {
                vm.register_native_property(receiver, &name,
...
fn charge(budget: &mut u64) -> Result<()> {
    *budget = budget.checked_sub(1).ok_or_else(|| unsupported("windowEx execution budget exceeded"))?;
    Ok(())
}
impl Services {
    fn notification_dictionary(&mut self, vm: &mut Vm) -> Result<Value> {
        let class = vm.globals.get("Window").cloned().context("cache setup failed.")?;
        ...
        for (name, number, reverse) in interface()?.notifications {
            vm.set_member(&dictionary, &Value::string(&name), Value::Integer(number.into()))?;
            vm.set_member(&dictionary, &Value::Integer(number.into()), Value::string(&reverse))?;
        }
        vm.set_member(&class, &key, dictionary.clone())?;
        Ok(dictionary)
    }
    fn notification_value(&mut self, vm: &mut Vm, key: &Value, budget: &mut u64) -> Result<Option<Value>> {
        ...
        Ok(Some(vm.get_property(&dictionary, key, false, false, self, budget)?))
    }
    pub(crate) fn window_ex_call(&mut self, vm: &mut Vm, operation: &str, context: &Value, args: &[Value], budget: &mut u64) -> Result<Value> {
        charge(budget)?;
        let arg = |i: usize| args.get(i).with_context(|| format!("windowEx {operation}: missing argument {i}"));
        match operation {
            "Window.getNotificationNum" => {
                let key = arg(0)?.unary("string")?;
                return Ok(Value::Integer(self.notification_value(vm, &key, budget)?.map(|v| v.integer()).transpose()?.unwrap_or(-1) as i32 as i64));
            }
            "Window.getNotificationName" => {
                let key = Value::Integer(arg(0)?.integer()? as i32 as i64);
                return self.notification_value(vm, &key, budget)?.unwrap_or(Value::Void).unary("string");
            }
        ...
        if let Some(name) = operation.strip_prefix("Window.") {
            let id = id(context)?;
            ensure!(self.windows.get(&id).is_some_and(|w| w.constructed), "context has no constructed Window native instance");
            if name == "registerExEvent" {
                self.window_ex.windows.entry(id).or_default();
                for (index, event) in ["onResizing", "onMoving", "onMove", "onNcMouseMove"].iter().enumerate() {
                    let key = Value::string(event);
                    let exists = vm.has_member(context, &key)?;
                    if exists {
                        vm.get_property(context, &key, false, false, self, budget)?;
                    }
                    self.window_ex.windows.get_mut(&id).context("Window invalidated during event registration")?.events[index] = exists;
                }
                return Ok(Value::Void);
            }
            if let Some(property) = name.strip_prefix("get:") {
                ...
                return Ok(match property {
                    ...
                    "disableResize" => Value::Integer(state.is_some_and(|s| s.disable_resize).into()),
                    "disableMove" => Value::Integer(state.is_some_and(|s| s.disable_move).into()),
                    "enableNCMouseEvent" => Value::Integer(state.is_some_and(|s| s.nc_mouse).into()),
                    "exSystemMenu" => if state.is_some() { Value::NULL } else { Value::Void },
                    _ => return Err(unsupported(format!("windowEx native window property: {property}"))),
                });
            }
            if matches!(name, "set:disableResize" | "set:disableMove" | "set:enableNCMouseEvent") {
                let state = self.window_ex.windows.get_mut(&id).context("Invalid operation for Read-only or Write-only property")?;
            ...
            if name == "setMessageHook" {
                ensure!(self.window_ex.windows.contains_key(&id), "Invalid operation for Read-only or Write-only property");
                ...
                let number = if let Some(key) = args.get(1) {
                    let number = if matches!(key, Value::String(_)) {
                        self.notification_value(vm, key, budget)?.map(|v| v.integer()).transpose()?.unwrap_or(-1)
                    } else { key.integer()? } as i32;
                    ensure!((0..1024).contains(&number), "Unknown failure : FFFFFFFF");
                    Some(number as usize)
                } else { None };
                let state = self.window_ex.windows.get_mut(&id).context("Window invalidated during notification lookup")?;
                if let Some(number) = number {
                    if on { state.hooks[number / 32] |= 1 << (number % 32); }
                    else { state.hooks[number / 32] &= !(1 << (number % 32)); }
                } else { state.hooks.fill(if on { u32::MAX } else { 0 }); }
