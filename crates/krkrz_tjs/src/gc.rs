...
impl ObjectArena {
    pub fn new(global: Object) -> Self {
        Self {
            entries: BTreeMap::from([(0, global)]),
            next: 1,
        }
    }
    ...
    pub fn push(&mut self, object: Object) {
        ...
        self.next = self
            .next
            .checked_add(1)
            .expect("TJS handle space exhausted");
    }
...
impl IndexMut<usize> for ObjectArena {
    fn index_mut(&mut self, id: usize) -> &mut Object {
        self.entries
            .get_mut(&id)
            .expect("validated TJS object handle")
    }
}
...
impl Vm {
    ...
    pub fn should_collect_garbage(&self) -> bool {
        self.allocations_since_gc >= 8192
    }
    ...
    fn mark_objects(
        ...
        for value in self
            .globals
            .values()
            .chain(self.literal_objects.values())
            .chain(roots)
        {
            references(value, &mut pending);
        }
        for value in host.gc_roots() {
            references(&value, &mut pending);
        }
        ...
        while let Some(id) = pending.pop() {
            ...
            *budget = budget
                .checked_sub(1)
                .ok_or_else(|| crate::unsupported("TJS collection budget exceeded"))?;
            let object = self
                .objects
                .get(id)
                .ok_or_else(|| anyhow::anyhow!("invalid TJS collection root: {id}"))?;
            pending.extend(object.owner);
            for value in object.members.values() {
                references(value, &mut pending);
            }
            match &object.kind {
                ...
                ObjectKind::Property { getter, setter } => {
                    for value in getter.iter().chain(setter) {
                        references(value, &mut pending);
                    }
                }
                ObjectKind::VariantProperty(value)
                | ObjectKind::Method {
                    receiver: value, ..
                } => references(value, &mut pending),
                ...
                ObjectKind::ReadOnly(view) => {
                    ...
                    while let Some(tree) = trees.pop() {
                        if !readonly_seen.insert(Arc::as_ptr(&tree) as usize) {
                            continue;
                        }
                        *budget = budget
                            .checked_sub(1)
                        ...
                        match tree.as_ref() {
                            ...
                            ReadOnlyData::Dictionary(items) => {
                                trees.extend(items.values().cloned())
                            }
                        }
                    }
                }
    ...
    pub fn collect_garbage(
        ...
        budget: &mut u64,
            ...
            self.depth == 0 && !self.collecting,
            "TJS collection requires an idle interpreter"
        );
    ...
    fn collect_idle(
        ...
        for &id in &dead {
            ...
            if object.valid
                && (matches!(object.kind, ObjectKind::Instance)
                    || !object.native_finalizers.is_empty())
            {
...
mod tests {
    ...
    fn eval(vm: &mut Vm, source: &str) -> Value {
        vm.execute(
            &compile_expression("gc", source).unwrap(),
            &mut (),
            &mut 10_000_000,
        )
        .unwrap()
    ...
    fn reclaims_cycles_preserves_explicit_roots_and_never_reuses_ids() -> Result<()> {
        ...
        vm.execute(
            &compile("gc", "for(var i=0;i<2000;i++){var a=[];a.add(a);}")?,
            &mut (),
            &mut 1_000_000,
        )?;
        assert!(
            vm.collect_garbage(&mut (), std::slice::from_ref(&retained), &mut 1_000_000)? >= 2000
        );
        assert_eq!(
            vm.get_member(&retained, &Value::Integer(1), false)?,
            Value::Integer(2)
        );
    ...
    fn collection_calls_finalizers_and_traces_native_edges() -> Result<()> {
        ...
        impl Host for Native {
            fn call(&mut self, _: &mut Vm, _: &str, _: &[Value], _: &mut u64) -> Result<Value> {
                unreachable!()
            }
        ...
        let mut host = Native {
            root: vm.allocate(ObjectKind::Namespace)?,
            edge: eval(&mut vm, "[123]"),
        };
        vm.collect_garbage(&mut host, &[], &mut 1_000_000)?;
        assert_eq!(vm.globals["done"], Value::Integer(1));
        assert_eq!(
            vm.get_member(&host.edge, &Value::Integer(0), false)?,
            Value::Integer(123)
        );
        vm.collect_garbage(&mut host, &[], &mut 1_000_000)?;
        assert_eq!(vm.globals["done"], Value::Integer(1));
    ...
    fn traces_bound_contexts_and_readonly_scalar_objects() -> Result<()> {
        ...
        let bound = eval(
            &mut vm,
            "(function(){return this.answer;}) incontextof %[answer:42]",
        );
        let leaf = eval(&mut vm, "[99]");
        let tree = vm.new_readonly_data(
            Arc::new(ReadOnlyData::Array(vec![Arc::new(ReadOnlyData::Scalar(
                leaf,
            ))])),
        ...
        assert_eq!(
            vm.invoke(&bound, &Value::NULL, &[], &mut (), &mut 1000)?,
            Value::Integer(42)
        );
        let leaf = vm.get_member(&tree, &Value::Integer(0), false)?;
        assert_eq!(
            vm.get_member(&leaf, &Value::Integer(0), false)?,
            Value::Integer(99)
        );
        Ok(())
    }
