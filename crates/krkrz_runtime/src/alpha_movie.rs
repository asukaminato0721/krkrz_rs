...
impl Services {
    pub(crate) fn alpha_movie_call(
        ...
        if operation == "showNextImage" {
            let target = args
                .first()
                .context("AlphaMovie.showNextImage requires a Layer")?;
            let Value::Object(target) = target else {
                anyhow::bail!("AlphaMovie.showNextImage requires a Layer");
            };
            let target = target
                .object
                .context("AlphaMovie.showNextImage requires a non-null Layer")?;
            ensure!(
                self.layers.contains_key(&target),
        ...
        let integer = match operation {
            ...
            "setPosition" => {
                    ...
                    args.len() >= 2,
                    "AlphaMovie.setPosition requires two coordinates"
                );
