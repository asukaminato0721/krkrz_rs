...
fn session(source: &str) -> (Session, tempfile::TempDir, tempfile::TempDir) {
    ...
    std::fs::write(project.path().join("tone.wav"), include_bytes!("fixtures/tone.wav")).unwrap();
    std::fs::write(project.path().join("startup.tjs"), format!(r#"
...
{source}
"#)).unwrap();
    let mut session = Session::open(project.path(), Some(saves.path()), None, 2_000_000).unwrap();
...
fn plugin_link_preserves_function_and_silent_graph_pixels() {
    let (mut s, _project, _saves) = session(r#"
...
"#);
    assert_eq!(s.evaluate("original===drawFFTGraph && typeof result=='void'").unwrap(), Value::Integer(1));
    assert_eq!(s.evaluate("updates.join('|')").unwrap(), Value::string("2,3,8,8"));
    ...
    s.evaluate("drawFFTGraph(l,s,2,3,8,8,%[type:1,division:2,thick:2])").unwrap();
    assert_eq!(s.evaluate("[l.getMainPixel(2,10),l.getMainPixel(3,9),l.getMainPixel(3,7)].join(',')").unwrap(), Value::string("12632256,7368816,11579568"));
}
...
var signal=highest();
"#);
    assert_eq!(s.evaluate("signal>0 && s.samplePosition==0").unwrap(), Value::Integer(1));
    ...
    s.evaluate("Scripts.exec('for(var i=0;i<70;i++)drawFFTGraph(l,s,0,0,64,32);')").unwrap();
...
fn unsafe_rectangles_options_and_reentrant_invalidation_are_rejected() {
    ...
    for call in [
        ...
        "drawFFTGraph(l,%[getVisBuffer:function(){invalidate l;return 0;}],0,0,8,8)",
    ] {
...
fn callback_budget_exhaustion_is_not_swallowed() {
    let (mut s, _project, _saves) = session("");
    s.budget = 30_000;
    let error = s.evaluate("drawFFTGraph(l,%[getVisBuffer:function(){while(true){}}],0,0,8,8)").unwrap_err();
    assert!(error.downcast_ref::<krkrz_tjs::VmAbort>().is_some(), "{error:#}");
}
