...
fn movie_session() -> (Session, tempfile::TempDir, tempfile::TempDir) {
    let project = tempfile::tempdir().unwrap();
    let saves = tempfile::tempdir().unwrap();
    std::fs::write(project.path().join("movie.mpg"),include_bytes!("fixtures/video_overlay.mpg")).unwrap();
...
fn movie_frames_pcm_pause_seek_and_eof_use_session_clock() {
    ...
    assert_eq!(session.evaluate("v.originalWidth+\",\"+v.originalHeight+\",\"+v.fps").unwrap(),Value::string("32,24,25"));
    let color = session.evaluate("l.getMainPixel(16,12)").unwrap().integer().unwrap();
    assert!((color & 0xff0000) > 0xf00000,"decoded red pixel: {color:x}");
    ...
    assert_eq!(session.evaluate("events.join(\",\")").unwrap(),Value::string("stop,play,pause,play,stop"));
    session.tick(840).unwrap();
    assert!(session.take_audio().is_empty());
    session.evaluate("v.close()").unwrap();
    session.dispatch_events().unwrap();
    assert_eq!(session.evaluate("events[events.count-1]").unwrap(),Value::string("unload"));
}
#[test]
fn movie_loop_and_period_callbacks_follow_frame_progress() {
    let (mut session,_project,_saves) = movie_session();
    session.evaluate("Scripts.exec('v.setPeriodEvent(2);v.loop=true;v.play();')").unwrap();
    session.tick(120).unwrap();
    assert_eq!(session.evaluate("v.periodEventFrame").unwrap(),Value::Integer(-1));
    session.tick(480).unwrap();
    assert_eq!(session.evaluate("events.join(\",\")").unwrap(),Value::string("stop,play,period1,period0"));
    session.evaluate("Scripts.exec('v.loop=false;v.setSegmentLoop(1,3);v.rewind();')").unwrap();
    session.tick(640).unwrap();
    assert_eq!(session.evaluate("v.frame").unwrap(),Value::Integer(2));
    assert_eq!(session.evaluate("events[events.count-1]").unwrap(),Value::string("period3"));
}
...
fn installed_movie_decodes_seeks_and_finishes() {
    ...
    session.evaluate(&format!("v.position={}",duration/2)).unwrap();
    session.tick(200).unwrap();
    session.evaluate(&format!("v.position={}",duration.saturating_sub(50))).unwrap();
