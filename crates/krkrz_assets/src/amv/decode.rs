...
impl Bits<'_> {
    ...
    fn symbol(&mut self, counts: &[u8; 16], values: &[u8]) -> Result<u8> {
        ...
        for &count in counts {
            ...
            if code >= first && code < first + count as i32 {
                return Ok(values[index + (code - first) as usize]);
            }
            index += count as usize;
            first = (first + count as i32) << 1;
        }
    ...
    fn block(&mut self, chroma: bool, predictor: &mut i32, quant: &[u16; 64]) -> Result<[u8; 64]> {
        let (dc_lengths, dc_values, ac_lengths, ac_values) = if chroma {
            (
                &CHROMA_DC_CODE_LENGTHS,
                &CHROMA_DC_VALUES,
                &CHROMA_AC_CODE_LENGTHS,
                &CHROMA_AC_VALUES,
            )
        } else {
            (
                &LUMA_DC_CODE_LENGTHS,
                &LUMA_DC_VALUES,
                &LUMA_AC_CODE_LENGTHS,
                &LUMA_AC_VALUES,
            )
        };
        let size = self.symbol(dc_lengths, dc_values)?;
        *predictor = predictor
            .checked_add(self.signed(size)?)
            .context("AlphaMovie DC predictor overflow")?;
        let mut coeff = [0i16; 64];
        coeff[0] = (*predictor)
            .try_into()
            .context("AlphaMovie DC coefficient overflow")?;
        let mut k = 1;
        while k < 64 {
            ...
            if symbol == 0xf0 {
                k += 16;
                ensure!(k <= 64, "AlphaMovie AC run exceeds block");
                continue;
            }
            k += (symbol >> 4) as usize;
...
impl Movie {
    ...
    pub fn decode_packet(&self, index: usize) -> Result<Option<Image>> {
        ...
        let tables: Vec<[u16; 64]> = self
            .quantization
            .iter()
            .map(|q| std::array::from_fn(|i| q[i] as u16))
            .collect();
        let alpha = if let Some(compressed) = self.compressed_alpha(index)? {
            let mut alpha = Vec::new();
            flate2::read::ZlibDecoder::new(compressed)
                .take((width * height + 1) as u64)
                .read_to_end(&mut alpha)?;
            ensure!(
                alpha.len() == width * height,
                "AlphaMovie alpha plane size mismatch"
            );
            Some(alpha)
        } else {
            None
        ...
        for my in (0..height).step_by(16) {
            for mx in (0..width).step_by(16) {
                ...
                let mut ys = [[0; 64]; 4];
                for block in &mut ys {
                    *block = bits.block(false, &mut luma, &tables[0])?;
                }
                let mut alphas = [[255; 64]; 4];
                if self.header.alpha_encoding == AlphaEncoding::Dct {
                    for block in &mut alphas {
                        *block = bits.block(false, &mut luma, &tables[2])?;
                    }
                }
                for y in 0..16 {
                    for x in 0..16 {
                        let block = (y / 8) * 2 + x / 8;
                        let at = (y % 8) * 8 + x % 8;
                        let yy = ys[block][at] as f64;
                        let u = cb[(y / 2) * 8 + x / 2] as f64 - 128.;
                        let v = cr[(y / 2) * 8 + x / 2] as f64 - 128.;
                        let pixel = (my + y) * width + mx + x;
